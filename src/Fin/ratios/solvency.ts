 ...
 */
export const RatiosDebtToAsset = ({
  totalAssets,
  totalDebt,
}: RatiosDebtToAssetParams): number | null => {
  if (
    !totalAssets
 ...
 */
export const RatiosNetDebtToAsset = ({
  totalAssets,
  netDebt,
}: RatiosNetDebtToAssetParams): number | null => {
  if (
    !totalAssets
    || isNullOrUndefined(netDebt)
  ) return null;
 ...
 */
export const RatiosNetDebtToNetAsset = ({
  shareholdersEquity,
  netDebt,
}: RatiosNetDebtToNetAssetParams): number | null => {
  if (
    isNullOrUndefined(netDebt)
    || !shareholdersEquity
  ) return null;
 ...
 */
export const RatiosNetDebtToEBITDA = ({
  netDebt,
  EBITDA,
}: RatiosNetDebtToEBITDAParams): number | null => {
  if (
    isNullOrUndefined(EBITDA)
    || isNullOrUndefined(netDebt)
    || EBITDA
  ) return null;
 ...
 */
export const RatiosNetDebtToEbit = ({
  netDebt,
  EBIT,
}: RatiosNetDebtToEbitParams): number | null => {
  if (
    isNullOrUndefined(netDebt)
...
export const RatiosDebtToEquity = ({
  shareholdersEquity,
  totalLiabilities,
}: RatiosDebtToEquityParams): number | null => {
  if (
    isNullOrUndefined(shareholdersEquity)
    || isNullOrUndefined(totalLiabilities)
  ) return null;
...
*/
export const RatiosInterestCoverage = ({
  interestExpenses,
  EBIT,
}: RatiosInterestCoverageParams): number | null => {
  if (
    !interestExpenses
    || isNullOrUndefined(EBIT)
  ) return null;
